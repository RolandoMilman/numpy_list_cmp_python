...
if __name__ == '__main__':
    ...
    # ['Tamara', 'Juan', 'Alberto'......]
    print()
    print('Código con ciclo FOR: ')
    for x in nombres:
        if(x[:1] in padron):
            print('El nombre:',x, 'está en la lista de padron porque comienza con la letra', x[:1])
        else:
            print('No existe registro con letra: ', x[:1])


    print()
    print('Código con comprensión de listas: ')
    nombres_en_padron = [x for x in nombres if(x[:1] in padron) is True]


    print('Nombres que figuran en el padrón:', nombres_en_padron)
